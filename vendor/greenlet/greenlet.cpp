...
*/
/*
 * filament vendoring guard: the vendored greenlet 3.5.4 is Python 3
 * only.  On Python 2 we compile a stub whose init raises ImportError,
 * so the Python layer's ``import _fil_greenlet`` falls back to the
 * installed classic greenlet (see filament/greenthread.py et al.).
 */
#define PY_SSIZE_T_CLEAN
#include <Python.h>
#if PY_MAJOR_VERSION < 3

extern "C" PyMODINIT_FUNC init_fil_greenlet(void);
PyMODINIT_FUNC init_fil_greenlet(void)
{
        ...
        */
        const NewReference c_api_object(Require(
                                           PyCapsule_New(
                                               ...
                                               "_fil_greenlet._C_API",
                                               NULL)));
        m.PyAddObject("_C_API", c_api_object);
        {
            PyObject* f = PyCFunction_New(&vgl_stats_def, NULL);
            PyModule_AddObject(m.borrow(), "vgl_stats", f);
            PyObject* fc = PyCapsule_New((void*)vgl_fast_switch,
...
/* --- benchmark instrumentation exports --- */
extern "C" {
...
{
    extern unsigned long long vgl_bytes_saved, vgl_bytes_restored, vgl_switch_count;
    return Py_BuildValue("KKK", vgl_bytes_saved, vgl_bytes_restored, vgl_switch_count);
}
...
extern "C" PyObject*
vgl_fast_switch(PyObject* self_)
{
    PyGreenlet* self = (PyGreenlet*)self_;
    using greenlet::SwitchingArgs;
    try {
        SwitchingArgs switch_args(OwnedObject::owning(mod_globs->empty_tuple),
                                  OwnedObject::owning(nullptr));
...
#endif /* PY_MAJOR_VERSION >= 3 (filament vendoring guard) */
